...
import com.inxpl.api.GetTranslationTask;
import com.inxpl.bean.DataBean;
import com.inxpl.utils.StringUtils;
 ...
 * @author liubin
 */
public class ExtractTextAction extends AnAction {
    ...
    public void actionPerformed(AnActionEvent e) {
        ...
        PsiFile[] files = psiFile.getParent().getFiles();

        for (int i = 0; i < files.length; i++) {
            PsiFile psiChildFile = files[i];
//            VirtualFile virtualFile = psiChildFile.getVirtualFile();
//            try {
//                virtualFile.rename(VirtualFileManager.VFS_CHANGES, "A" + files[i].getName());
//            } catch (IOException ex) {
//                ex.printStackTrace();
//            }

            logger.warn("====actionPerformed=====" + files[i].getName());
        }
        logger.warn("====actionPe开始变化rformed=====");

        VirtualFileManager.getInstance().asyncRefresh(new Runnable() {
            @Override
            public void run() {
                logger.warn("====action变化结束Performed=====");
    ...
    private void handLayout(String layoutName) {
        ...
        logger.warn("需要处理的布局文件为:" + layoutName);
